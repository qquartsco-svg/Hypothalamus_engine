...
**이 블록체인 기반 기여도 시스템 자체도 계속 업그레이드되어가는 구조입니다.**

현재는 초기 설계 단계이며, 실제 상용화가 이루어지고 경험이 쌓이면서 시스템이 발전하고 개선될 예정입니다. 구체적인 기여도 계산 방식이나 분배 메커니즘은 향후 커뮤니티와의 협의를 통해 결정됩니다.

### ⚠️ GNJz의 기여도 원칙 (블록체인 기반)

**최초 코드 작성자 GNJz (Qquarts)는 자신의 기여도가 총 기여도 중 6%를 넘지 않도록 제한합니다.**

이것은 **블록체인 기반 신뢰 시스템**의 원칙입니다:

- **코드 재사용**: 로열티를 요구하지 않습니다. 자유롭게 사용 가능합니다
- **수익 발생 시**: 이 코드를 사용하여 수익이 발생하면, 블록체인으로 계산된 기여도에 따라 수익이 분배됩니다
- **기여도 계산**: PHAM 블록체인의 4-Signal Scoring으로 객관적으로 계산됩니다
- **GNJz의 자발적 제한**: GNJz는 자신의 기여도가 6%를 넘지 않도록 스스로 제한합니다
- **신뢰 기반**: 이것은 "로열티를 6%로 받겠다"는 요구가 아니라, **블록체인으로 검증 가능한 기여도 상한선**입니다
- **투명성**: 모든 기여도 계산은 블록체인에 기록되어 누구나 검증할 수 있습니다
